 ...
 *   – TOP pools are ordered on NATURAL correlation; degenerate `NaN` correlations are allowed but ranked
 *     after all finite values (suffix) to keep deterministic ordering.
 */
...
class DatasetModelTest {
    ...
    /**
     * Light checks that names/tokens include correlation and key dimensions without heavy I/O.
     */
    @Test
    ...
    /**
     * Tiny synthetic dataset (3 systems × 4 topics); stream events and assert:
     ...
     * • Model cache equals the last emitted improvement per K.
