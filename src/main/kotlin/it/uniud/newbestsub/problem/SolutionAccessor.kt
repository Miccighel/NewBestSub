...
/**
 * Convenience extension forwarders for legacy call sites that still type solutions as [BinarySolution].
 *
 * These functions downcast to [BestSubsetSolution] to expose domain-specific metrics while keeping
 * older code (and generic jMetal types) compiling with a minimal change at the call site.
 *
 * ⚠️ Note: A runtime `ClassCastException` will be thrown if the receiver is not a [BestSubsetSolution].
