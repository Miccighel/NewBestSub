 ...
 *   derived externally (for example via Tools.stableSeedFrom(Parameters)).
 *
 ...
 * - 0 (default) means auto: the controller or model may map this to the available cores.
 * - A controller may set a specific value to avoid over-subscription when running
 *   multiple NSGA-II jobs concurrently (for example, split cores between BEST and WORST).
 *
 ...
 *
 * @property datasetName Dataset identifier (for example, collection name or tag).
 * @property correlationMethod Correlation function key (for example, "kendall", "spearman", "pearson").
 ...
 * @property percentiles Percentiles to compute/export for summaries (for example, [5, 50, 95]).
 ...
 * @property evaluatorThreads Size of the jMetal evaluator thread pool; 0 ⇒ auto (map to CPU cores).
 */
