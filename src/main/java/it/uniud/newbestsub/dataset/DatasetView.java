...
public class DatasetView extends AbstractAlgorithmRunner {
    ...
    public void print(ImmutablePair<List<Solution>, Long> runResult, String outputPath) {

        logger.log("VIEW - Starting to print the result");

        List population = runResult.left;
        long computingTime = runResult.right;

        SolutionListOutput populationHelper = new SolutionListOutput(population);
